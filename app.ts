...
} // 타입 가드 코드(null일 때 예외 처리)

function newsFeed(): void {
  let newsFeed: NewsFeed[] = store.feeds;
  const newsList = [];
  // 템플릿 => 명확한 구조 파악 가능, 복잡도 감소 가능
  let template = `
  ...
  if (newsFeed.length === 0) {
    newsFeed = store.feeds = makeFeeds(getData<NewsFeed[]>(NEWS_URL)); // 제네릭
  }
...
function makeComment(comments: NewsComment[]): string {
  ...
  for (let i = 0; i < comments.length; i++) {
    ...
    if (comment.comments.length > 0) {
      commentString.push(makeComment(comment.comments)); //끝을 알 수 없는 구조일 때 재귀 호출
    }
  }
